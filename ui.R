...
#   helpText("Power estimation associated with a logistic regression interaction term in the 2 x 2 x 2 case:"),
   ...
   helpText(
     h4(
       div("This is an R Shiny web application that estimates power associated with a logistic regression interaction term
        in the 2 x 2 x 2 case.  This means that the response variable has only two possible outcomes and that there are
        ...
        the two 2 x 2 tables.   Let X1=Yes be the characteristic of interest for X1, and let X2=1 be the characteristic
        of interest for X2.", style = "color:black")
     )
     ),
     ...
     column(2,
            tags$table(border="1", align="center",
              ...
              tags$tr(
                ...
                tags$th(style = "text-align:center", colspan="2", "Predictor (X1)"),
                tags$th(" ")
              ),
              tags$tr(
                tags$td(align="center", "No"),
                tags$td(align="center", "Yes"),
                tags$td(align="center", "Total")
              ),
              tags$tr(
                tags$th(align="center", rowspan="2", "Outcome"),
                tags$td(align="center", "No"),
  ...
  helpText(
    h4(
      div("Suppose there is some binary quality of care outcome of interest in long-term care facilities.   The main 
        predictor of interest (X1) is whether or not a person is obese, and an effect modifier of interest is a binary
        race variable (X2) where X2=1 corresponds to African-Americans and X2=0 corresponds to Caucasians.  Suppose that
        it is known that 60% of the population of interest is obese (regardless of race) and that 15% of the population
        of interest is African-American.  Further, one conjectures that the odds ratio associated with the binary
        outcome of interest with respect to the obesity predictor for the Caucasian group is 2 and that the corresponding odds ratio of interest for the African-American
        group is 4.  One would like to estimate the power to detect this 2-unit difference in the two odds ratios based
        on a likelihood ratio test approach (such as logistic regression).  To use this app, one needs to conjecture a
        proportionality constant, k1, relating cell counts b and d above (the cell counts for the 'Yes' group for X1 when
        X2=0) where d=k1*b and also a proportionality constant, k2, relating cell counts B and D (the cell counts for the 'Yes' group
  ...
  fluidRow(
    column(2,
         tags$table(border="1", align="center",
                    tags$caption(style = "text-align:center", strong(
                                        div("X2=Caucasian", style = "color:black")
                                        )
                                 ),
                    tags$tr(
                      tags$th(colspan="2", rowspan="2", " "),
                      tags$th(style = "text-align:center", colspan="2", "Predictor (Obese)"),
                      tags$th(" ")
                    ),
                    tags$tr(
                      tags$td(align="center", "No"),
    ...
    column(2,
         tags$table(border="1", align="center",
                    tags$caption(style = "text-align:center", strong(
                                        div("X2=African-American", style = "color:black")
                                        )
                                 ),
                    tags$tr(
                      tags$th(colspan="2", rowspan="2", " "),
